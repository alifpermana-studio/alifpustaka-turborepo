...
type Props = {
  deleteUser: userOnAdminType;
  reload: () => void;
  closeModal: () => void;
};

export const DeleteUser = ({ deleteUser, reload, closeModal }: Props) => {
    ...
    <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 lg:p-11 dark:bg-gray-900">
      <div className="px-2 pr-14">
        ...
        <p className="mb-6 text-sm text-gray-500 lg:mb-7 dark:text-gray-400">
          Deleted user will be removed automatically from our database if you do not restore it in 6
          month.
        </p>
      </div>
        ...
        <div className="flex w-full flex-col items-start justify-start gap-1 border-l-[1px] border-amber-500 pl-5 text-lg text-gray-800 dark:text-gray-200">
          <div className="flex flex-row gap-3">
            <span className="mr-1 block truncate font-medium text-ellipsis">
              {`${deleteUser.fname} ${deleteUser.lname}`}
            </span>
            <span>@{deleteUser.username}</span>
          </div>
          <div>{deleteUser.email}</div>
          <div className="mt-3 flex flex-row gap-5">
            <span
              className={`rounded px-2 py-1 text-sm font-semibold ${
                deleteUser.userRole === UserRole.SUPERADMIN
                  ? 'bg-red-800 text-gray-200'
                  : deleteUser.userRole === UserRole.ADMIN
                    ? 'bg-black text-white'
                    : deleteUser.userRole === UserRole.EDITOR
                      ? 'bg-purple-800 text-gray-200'
                      : deleteUser.userRole === UserRole.WRITER
            ...
            <span
              className={`rounded px-2 py-1 text-sm font-semibold ${
                deleteUser.userStatus === UserStatus.BANNED
                  ? 'bg-red-800 text-gray-200'
                  : deleteUser.userStatus === UserStatus.DELETED
                    ? 'bg-black text-white'
                    : deleteUser.userStatus === UserStatus.ACTIVE
                      ? 'bg-purple-800 text-gray-200'
                      : 'bg-yellow-500 text-gray-800'
              }`}
            >
