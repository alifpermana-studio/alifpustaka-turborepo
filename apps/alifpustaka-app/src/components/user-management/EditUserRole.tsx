...
export const EditUserRole = ({ isOpen, editUser, reload, closeModal }: Props) => {
  ...
  useEffect(() => {
    if (formData.roleReq.id && isOpen) {
      setFormData(prev=>({ ...prev, roleReq: { ...prev.roleReq, resUserRole: prev.roleReq.reqUserRole } }));
    } else {
    ...
    <div className="no-scrollbar relative w-full overflow-y-auto rounded-3xl bg-white p-4 lg:p-11 dark:bg-gray-900">
      <div className="px-2 pr-14">
        ...
        <p className="mb-6 text-sm text-gray-500 lg:mb-7 dark:text-gray-400">
          This status handle how to assign user role.
        </p>
      </div>
        ...
        <div className="flex w-full flex-col items-center justify-center gap-3 md:justify-between">
          <div className="flex w-full flex-col gap-3">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <h6>Current Role </h6>
              <div>{editUser.userRole}</div>
            </div>
            {formData.roleReq.id ? (
              <>
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <h6>Request Role </h6>
                  <div>{formData.roleReq.reqUserRole}</div>
                </div>
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <h6>User Message </h6>
                  <div>"{formData.roleReq.reqMsg}"</div>
                </div>
                <div className="col-span-2 grid gap-2">
                  <p>Your response message</p>
                  <Input
                    placeholder="Because the user ..."
                    onChange={(e) =>
                      setFormData((prev) => ({
                        ...prev,
                        roleReq: { ...prev.roleReq, resMsg: e.target.value },
                      }))
                    }
                  />
                </div>
              ...
              <div className="grid grid-cols-1 items-center gap-3 md:grid-cols-2">
                ...
                <div className="col-span-2 grid gap-2">
                  <p>Why you want to change user role?</p>
                  <Input
                    placeholder="Because the user ..."
                    onChange={(e) =>
                      setFormData((prev) => ({
